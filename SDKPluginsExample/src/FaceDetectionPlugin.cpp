...
void FaceDetectionPlugin::cameraFrameAvailable(const wikitude::sdk::Frame& cameraFrame_) {
    ...
    if ( 1 == _flipFlag ) { /* flip flag 1 == device orientation portrait */
        cv::transpose(smallImg, smallImg);
        cv::flip(smallImg, smallImg, _flipFlag);
    } else if ( 0 == _flipFlag ) { /* flip flag 0 == device orientation portrait upside down */
        cv::transpose(smallImg, smallImg);
        cv::flip(smallImg, smallImg, _flipFlag);
    } else if ( -1 == _flipFlag ) { /* flip flag -1 == device orientation landscape left */
        cv::flip(smallImg, smallImg, _flipFlag);
    }
...
void FaceDetectionPlugin::setFlipFlag(int flipFlag_) {
    _flipFlag = flipFlag_;
    calculateProjection(_flipFlag, -1.f, 1.f, -1.f, 1.f, 0.f, 500.f);
    _observer->projectionMatrixChanged(_projectionMatrix);
}

void FaceDetectionPlugin::convertFaceRectToModelViewMatrix(cv::Mat& frame_, cv::Rect& faceRect_) {
    ...
    if ( 1 == _flipFlag || 0 == _flipFlag ) {
        x = (xtmp / _cameraFrameWidth ) - 0.5f * _aspectRatio;
        y = (ytmp / _cameraFrameWidth ) - 0.5f;
        
        scaleX = ((float)faceRect_.width / (float)frame_.cols * _scaledCameraFrameWidth);
        ...
        scaleY = ((float)faceRect_.height / (float)frame_.cols * _scaledCameraFrameHeight);
    }
...
void FaceDetectionPlugin::calculateProjection(int flipFlag, float left, float right, float bottom, float top, float near, float far) {

    if ( 1 == flipFlag || 0 == flipFlag ) { /* portrait projection */
        left *= _aspectRatio;
        right *= _aspectRatio;
    } else {
        top *= _aspectRatio;
